...
def result(data_ex , params):
    ...
    for period in range(num):
        ...
        #ログファイルの初期化
        model_function.initialize_log_files(params["seeds"],period_log)
        ...
        if flag_skip == 0:
            ...
            model = model_function.search_optimal_units(params,train_data,period_log)
        ...
        if flag_skip == 0:
            # データセットへの格納
            # 特徴量ごとのトレーニングデータに対する再構成誤差の平均
            reconstraction_train_per_features.loc["period"+str(period_log)] = errors_train_per_features_ave

            # 閾値データの特徴量ごとの再構成誤差
            thresholds_per_features.loc["period"+str(period_log)] = threshold_per_features
        ...
        # 特徴量ごとのテストデータに対する再構成誤差の平均
        reconstraction_test_per_features.loc["period"+str(period_log)] = errors_predict_per_features_ave 
