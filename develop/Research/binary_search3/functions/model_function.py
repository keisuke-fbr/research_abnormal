...
def calculate_thresholds(train_data_original, train_data_normalized, columns_list):
    ...
    """
    thresholds = {}
    # 正規化データが未指定の場合は正規化前データを代用
    if train_data_normalized is None:
        train_data_normalized = train_data_original
    ...
    for i, col_name in enumerate(columns_list):
        ...
        # 基準値 = 10^(-有効桁数)
        threshold_sig = 10 ** (-significant_digits)
        # 正規化後データの差分からノイズ分散を推定
        diffs = np.diff(train_data_normalized[:, i])
        if diffs.size < 2:
            noise_var = 0.0
        else:
            diff_mean = np.mean(diffs)
            noise_var = np.sum((diffs - diff_mean) ** 2) / (diffs.size - 1)
        threshold_noise = np.sqrt(0.5 * noise_var)
        # 有効数字基準とノイズ基準の大きい方を採用
        threshold = max(threshold_sig, threshold_noise)
        thresholds[col_name] = threshold

        # デバッグ用出力
        print(f"特徴量: {col_name}, 最大値: {max_value}, 有効桁数: {significant_digits}, 有効数字基準値: {threshold_sig}, ノイズ基準値: {threshold_noise}, 採用基準値: {threshold}")
...
def save_model_weights(model, seed, period_log, unit13, unit2, base_path):
    """
    最終モデルの重みをCSVに保存
    """
    if model is None:
        print("重み保存: モデルがNoneのため保存をスキップします。")
        return
    if seed is None:
        seed = -1
        print("重み保存: seedが取得できないため -1 を使用します。")

    weight_base_path = base_path
    if os.path.basename(weight_base_path) == "learning_scores":
        weight_base_path = os.path.dirname(weight_base_path)
    weight_dir = os.path.join(weight_base_path, "weight")
    os.makedirs(weight_dir, exist_ok=True)
    file_path = os.path.join(
        weight_dir,
        f"weight_seed{seed}_period{period_log}_unit13_{unit13}_unit2_{unit2}.csv"
    )
    ...
    for i in range(0, len(weights), 2):
        ...
        if w.ndim != 2:
            continue
        name = layer_names[layer_index] if layer_index < len(layer_names) else f"layer_{layer_index + 1}"
        for from_idx in range(w.shape[0]):
            for to_idx in range(w.shape[1]):
                rows.append({
                    "layer_index": layer_index + 1,
                    "layer_name": name,
                    "from_node": from_idx + 1,
                    "to_node": to_idx + 1,
                    "weight": float(w[from_idx, to_idx]),
                })
        layer_index += 1

    df = pd.DataFrame(rows)
    df.to_csv(file_path, index=False)
    print(f"重み保存: {file_path}")
...
def learn_model(params, model, train_data, thresholds, seed, period_log, unit13, unit2, base_path):
    ...
    early_stopping_params = {
        ...
        # 早期終了条件（基準未達特徴の最小最大誤差が改善しない場合）
        "min_improve": 0.005,
        "no_improve_patience": 10000
    }
...
def search_optimal_units(params, train_data, period_log, thresholds, base_path):
    ...
    if best_model is None:
        print("❗ 最適モデルが見つかりませんでした。最後に探索したモデルを返します。")
        seed = getattr(model, "selected_seed", None)
        save_model_weights(model, seed, period_log, unit13, unit_2, base_path)
        return model
    ...
    seed = getattr(best_model, "selected_seed", None)
    save_model_weights(best_model, seed, period_log, best_unit_1_3, best_unit_2, base_path)
    return best_model
