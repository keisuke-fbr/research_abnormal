    ...
    for period in range(num):
        ...
        if flag_skip == 0:
            ...
            if params.get("test_mode", False):
                ...
                # 通常モード：データから基準値を計算
                thresholds = model_function.calculate_thresholds(train_data_original, train_data, config.columns_list)
            # 各実験ごとの学習済みモデルが返される
        ...
        if flag_skip == 0:
            ...
            # 重みの保存
            model_function.save_model_weights(model, base_path, period_log)
