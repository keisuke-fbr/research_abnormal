...
def calculate_noise_threshold(data, column_index):
    """
    ノイズ条件に基づく基準値を計算
    
    時系列データの差分からノイズの標準偏差を推定する
    
    入力:
        data: ndarray - 正規化済みトレーニングデータ（形状: [N, 特徴数]）
        column_index: int - 対象特徴量のインデックス
    出力:
        sigma_noise: float - ノイズの標準偏差（基準値として使用）
    """
    # 対象特徴量の時系列データを取得
    x = data[:, column_index]
    
    # 差分を計算: Δx_t = x_t - x_{t-1}
    delta_x = np.diff(x)  # 形状: [N-1]
    
    # 差分の平均: Δx̄ = (1/(T-1)) * Σ Δx_t
    delta_x_mean = np.mean(delta_x)
    
    # 差分の分散: Var(Δx) = (1/(T-2)) * Σ (Δx_t - Δx̄)²
    # np.var with ddof=1 gives 1/(N-1), we need 1/(T-2) where T is original length
    # Since delta_x has length T-1, we use ddof=1 to get 1/(T-2)
    T = len(x)
    if T <= 2:
        # データが少なすぎる場合は0を返す
        return 0.0
    
    var_delta_x = np.sum((delta_x - delta_x_mean) ** 2) / (T - 2)
    
    # ノイズ分散: σ_noise² ≈ (1/2) * Var(Δx)
    var_noise = var_delta_x / 2
...
def calculate_thresholds(train_data_original, train_data_normalized, columns_list):
    """
    各特徴量の基準値を計算
    有効数字ベースとノイズ条件ベースの2つの基準値のmaxを採用
    
    入力:
        train_data_original: ndarray - 正規化前トレーニングデータ（有効数字計算用）
        train_data_normalized: ndarray - 正規化済みトレーニングデータ（ノイズ計算用）
    ...
    for i, col_name in enumerate(columns_list):
        ...
        threshold_significant = 10 ** (-significant_digits)
        
        # === ノイズ条件ベースの基準値 ===
        threshold_noise = calculate_noise_threshold(train_data_normalized, i)
        
        # === 最終的な基準値（2つのmax） ===
        threshold = max(threshold_significant, threshold_noise)
        thresholds[col_name] = threshold

        # デバッグ用出力
        print(f"特徴量: {col_name},最大値: {max_value}, 有効桁数: {significant_digits}, 有効数字基準値: {threshold_significant}, ノイズ基準値: {threshold_noise:.6f}, 採用基準値: {threshold:.6f}")
...
def learn_model(params, model, train_data, thresholds, seed, period_log, unit13, unit2, base_path):
    ...
    early_stopping_params = {
        ...
        "stagnation_epochs": params.get("stagnation_epochs", 10000),
        "stagnation_threshold": params.get("stagnation_threshold", 0.005)
    }
    ...
    # 停止情報を取得
    stop_info = early_stopping.get_stop_info()
    
    return model, stop_info
...
def run_one_seed(params, train_data, unit13, unit2, period_log, thresholds, base_path, init_num):
    ...
    if hasattr(model, 'reached_threshold') and model.reached_threshold:
        return model, True, seed, stop_info
    else:
        return model, False, seed, stop_info


def try_init_point(params, train_data, unit13, unit2, period_log, thresholds, base_path):
    """
    複数シードでの並列学習・成功判定
    有効なモデルが見つかったら他のプロセスを即座に停止
    ...
    # マルチプロセスでの実行
    pool = multiprocessing.Pool(processes=num_retry)
    ...
    try:
        ...
        for model, success, seed, stop_info in result_iter:
            last_model = model
            
            # 停止情報を出力
            if stop_info["stop_reason"] == "stagnation":
                print(f"⚠️ 学習停滞により終了 (seed={seed})")
                print(f"   - 終了エポック: {stop_info['stopped_epoch']}")
                print(f"   - 原因特徴: {stop_info['stagnant_feature']}")
                print(f"   - 停滞条件: {stop_info['stagnation_epochs']}エポックで{stop_info['stagnation_threshold']}以上の改善なし")
...
def save_model_weights(model, base_path, period_log):
    """
    モデルの重みとバイアスをCSVに保存
    ...
    # ファイルパス
    file_path = os.path.join(weights_dir, f"weights_period{period_log}.csv")
    ...
    for layer in model.layers:
        ...
        if len(weights) == 0:
            continue  # 重みを持たない層はスキップ
        
        layer_index += 1
        
        # weights[0]: 重み行列（形状: [入力ノード数, 出力ノード数]）
        # weights[1]: バイアスベクトル（形状: [出力ノード数]）
        weight_matrix = weights[0]
        bias_vector = weights[1] if len(weights) > 1 else None
        
        # 重みを1行1エッジ形式で保存
        from_nodes, to_nodes = weight_matrix.shape
        for from_node in range(from_nodes):
            for to_node in range(to_nodes):
                weight_data.append({
                    "layer_index": layer_index,
                    "from_node": from_node + 1,  # 1始まり
                    "to_node": to_node + 1,      # 1始まり
                    "weight": weight_matrix[from_node, to_node],
                    "type": "weight"
                })
        
        # バイアスを保存（from_node=0として表現）
        if bias_vector is not None:
            for to_node in range(len(bias_vector)):
                weight_data.append({
                    "layer_index": layer_index,
                    "from_node": 0,              # バイアスはfrom_node=0
                    "to_node": to_node + 1,      # 1始まり
                    "weight": bias_vector[to_node],
                    "type": "bias"
                })
