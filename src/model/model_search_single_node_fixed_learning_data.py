...
from src import model_basic_single_node_fixed_learning_data
import config
...
def result(data_ex , params):
    ...
    for period in range(num):
        ...
        if flag_skip == 0:
            ...
            model = model_basic_single_node_fixed_learning_data.single_model(params,train_data,period)
        ...
        if flag_skip == 0:
            #必要データセットの受け取り
            abnormal_score, threshold_data, errors_train_per_features_ave, errors_predict_per_features_ave, threshold_per_features, reconstract_error, errors_train_per_data = model_basic_single_node_fixed_learning_data.calcurate_data(train_data, test_data, train_predict_data, test_predict_data)
        else:
            abnormal_score, errors_predict_per_features_ave = model_basic_single_node_fixed_learning_data.calcurate_data(train_data, test_data, train_predict_data, test_predict_data, flag_skip)
