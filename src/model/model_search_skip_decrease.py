...
from src import model_basic_search_skip_decrease
import config
...
def result(data_ex , params):
    ...
    for period in range(num):
        ...
        model = model_basic_search_skip_decrease.search_model(params,train_data, period)

        print(str(period+1) + "回目の期間のモデル作成終了")

        #モデルが見つからなかった場合終了する
        if model == None:
            print("モデルは一つも見つかりませんでした")
            break
        ...
        #必要データセットの受け取り
        abnormal_score, threshold_data, errors_train_per_features_ave, errors_predict_per_features_ave, threshold_per_features, reconstract_error = model_basic_search_skip_decrease.calcurate_data(train_data, test_data, train_predict_data, test_predict_data)
